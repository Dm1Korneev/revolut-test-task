...
describe('ValueInput component', () => {
  ...
  test('change value', () => {
    const { getByLabelText } = render(<ValueInput {...props} />);
    fireEvent.change(getByLabelText('Test label'), { target: { value: '200.01', valueAsNumber: 200.01 } });
    expect(changeValue).toHaveBeenCalledWith(200.01);
  });

  test('change value to null when input is clear', () => {
    const { getByLabelText } = render(<ValueInput {...props} />);
    fireEvent.change(getByLabelText('Test label'), { target: { value: '', valueAsNumber: Number.NaN } });
    expect(changeValue).toHaveBeenCalledWith(null);
  });

  test('change value positive when inter negative', () => {
    const { getByLabelText } = render(<ValueInput {...props} />);
    fireEvent.change(getByLabelText('Test label'), { target: { value: '-5', valueAsNumber: -5 } });
